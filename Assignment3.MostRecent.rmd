...
```{r}
# reinstalling to make patchwork work 
...
# might need to install and load the brobdingnag package if plot doesn't work (sometimes over rides rethinking plot)
...
```{r}
# Before even building the model we can look at pair.wise comparisons to see correlation. 
# We find evidence for a G-factor of intelligence, but the G-factor can't tell much about the individual parts of IQ. 
# V ~ Nv = .67
# V ~ S = .6
# Nv ~ S = .37 

# This means there is shared variance between V & Nv IQ that is not shared between Social IQ, so makes sense to include at least 2. 
# However, we see by adding information in our plots that adding additional information gives better explined variance in the model compared to only including 2. 
# Can also look at beta posterior of single predictor, and then see how much it moves when we add variance. 
...
# indentifying the outlier in the data 
identify( x=d$ScaleADOS , y=mu.mean , labels=d$ChildID , cex=0.8 )
# press esc after selecting points
```
...
It doesn't make much sense anymore since TD children practically all have ADOS scores of 0. This means the correlation between ADOS and any measures for the TD children will be close to 0. The model would likely still show a correlation between the IQ measures and ADOS, but the uncertainty will be much bigger and the slope flatter. So we will build exactly the same model as in 2.2, but on the whole dataset (still at visit 1). 
...
#reloading libraries becuase these packages suck and sometimes just stop working and then I can't get preety plots :( 
library(rethinking)
library(Brobdingnag)
precis(m1.6)
plot(precis(m1.6))
